...
public class CatalogListAdapter extends BaseAdapter {
    ...
    public void setLoading(boolean loading) {
        if ( loading ) {
            if ( feed.getSize() > 0 &&  !feed.getEntries().contains(this.loadingEntry) ) {
                ...
                lastEntry.getFeed().forEach((Command<? super Feed>) f -> this.loadingEntry.setFeed(f));
            }
        } else {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if ( convertView == null || convertView instanceof  ProgressBar  ) {
            ...
            rowView = inflater.inflate(net.zorgblub.typhonkai.R.layout.catalog_item, parent, false);
        } else {
            rowView = convertView;
        }
        ...
        ImageView icon = (ImageView) rowView.findViewById(net.zorgblub.typhonkai.R.id.itemIcon);
        ...
        icon.setImageDrawable( drawableOption.getOrElse(
                context.getResources().getDrawable(net.zorgblub.typhonkai.R.drawable.unknown_cover)
        ));
    ...
    private Option<Drawable> getThumbnail( Entry entry ) {
        ...
        final Option<Link> imgLink = Catalog.getImageLink( feed.unsafeGet(), entry );
